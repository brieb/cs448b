...
def get_filename(college_id, page):
  return '../data/college_board_dump2/' + \
...
def store_data(data):
  ...
  loc_data = get_data_for_address(data['college']['address'])
  ...
  q = "insert or ignore into \
      ...
      values("+ \
        ...
        "," + str(loc_data['latitude']) +\
        "," + str(loc_data['longitude']) +\
        "," + to_str(loc_data['gmap_lat_long_address']) +\
      ")"
  print q
  c.execute(q)
  ...
  for m in data['majors']:
    q = "insert or ignore into major(name)\
        values(\""+m['name']+"\")"
    c.execute(q)

    q = "insert or ignore into college_major(college_id, major_id, degree_type)\
        values("+college_id+",\
